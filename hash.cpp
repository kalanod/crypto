...
public:
    ...
    void read(char string[1024], int l);

    void read(unsigned char *string, int l);

    void print();

    void *getData(char &string1);

    void convertToHexArray(unsigned char *outputArray);
};
...
void *GOST::getData(char &string1) {
    sprintf(&string1, "%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x\0", result);
    return nullptr;
}
void GOST::convertToHexArray(unsigned char* outputArray) {

    for (int i = 0; i < 32; ++i) {
        // Преобразуем каждый байт в 16-ричное представление и записываем его в выходной массив
        std::sprintf(reinterpret_cast<char*>(outputArray) + i * 2, "%02x", result[i]);
    }
    outputArray[63]='\0';
...
void GOST::read(unsigned char *string, int l) {
    buf = string;
    len = l;
}
